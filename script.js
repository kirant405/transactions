    ...
    function renderAccounts(){
      ...
      data.accounts.forEach(acc=>{
        ...
        left.innerHTML = `<i class="fas fa-wallet"></i> ${acc}`;
        left.style.flex = '1';
    ...
    function renderTxItems(filtered){
      ...
      arr.forEach((tx, idx)=>{
        ...
        const categoryText = tx.category ? `<span class="txAccounts">${tx.category}${tx.subCategory ? ` → ${tx.subCategory}` : ''}</span>` : '';
        ...
        li.innerHTML = `<div style="display:flex;align-items:center;gap:10px;flex:1;flex-wrap:wrap;">
            ...
            <span style="min-width:80px">${tx.date}</span>
            <span style="flex:1">${tx.desc || '<span class="muted">No description</span>'}${recurringBadge}${categoryText ? `<br>${categoryText}` : ''}${recurringInfo}</span>
            <span style="min-width:90px"><span class="txAmount ${tx.type}">${formatCurrency(tx.amount)}</span></span>
            <span style="min-width:90px">${cbText || '<span class="muted">-</span>'}</span>
